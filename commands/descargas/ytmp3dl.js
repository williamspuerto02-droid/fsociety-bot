...
function resolveMaxAudioBytes(ctx) {
  const policy = getDownloadExecutionPolicy(ctx, "ytmp3dl");
  return Math.max(
    MIN_AUDIO_BYTES,
...
async function getYtmp3Data(videoUrl) {
  ...
  return {
    ...
    provider: data.provider || "ytmp3dl",
...
async function requestYtmp3FileDownload(videoUrl) {
  const { response } = await callYtmp3Api({
    videoUrl,
    mode: "file",
    responseType: "stream",
...
async function downloadYtmp3Fallback(videoUrl, preferredName, knownLinkData = null, options = {}) {
  ...
  const attempts = [
    async () => {
      const response = await requestYtmp3FileDownload(videoUrl);
...
export default {
  ...
  categoria: "descarga",
  category: "descarga",
  description: "Descarga audio MP3 de YouTube usando ytmp3dl rapido",
  ...
  run: async (ctx) => {
    ...
    const downloadPolicy = getDownloadExecutionPolicy(ctx, "ytmp3dl");
    ...
    try {
      ...
      const finalData = {
        ...
        sourceUrl: apiData.sourceUrl || getApiCandidates()[0] || "https://dv-yer-api.online/ytmp3dl",
      };
